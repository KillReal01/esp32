...
#define ESP_WIFI_PASS "esp32pass"
...
esp_err_t access_point_start_softap(void)
{
    ...
    std::memcpy(wifi_config.ap.ssid, ESP_WIFI_SSID, std::strlen(ESP_WIFI_SSID));
    std::memcpy(wifi_config.ap.password, ESP_WIFI_PASS, std::strlen(ESP_WIFI_PASS));
    wifi_config.ap.ssid_len = std::strlen(ESP_WIFI_SSID);
    wifi_config.ap.channel = 1;
    wifi_config.ap.max_connection = MAX_STA_CONN;
    wifi_config.ap.authmode = WIFI_AUTH_WPA_WPA2_PSK;

    if (std::strlen(ESP_WIFI_PASS) == 0) {
    ...
    ESP_LOGW(TAG, "SoftAP started. SSID:%s password:%s", ESP_WIFI_SSID, ESP_WIFI_PASS);
    return ESP_OK;
}
